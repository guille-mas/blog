...
However you can register the domain with the ip of the docker host at /etc/hosts, by using **docker exec** 
or implementing a command to be executed every time the container starts as I'll show you later. 
...
Containers are atomic infrastructure units, that must interact with a varying context: a cloud service, a localhost, a dedicated server, etc.
In order to provide the required flexibility, configuration settings must be initialized at the highest possible level, which is the environment itself.  
...
Any environment variable defined this way, can be overriden at run time, using the **-e VAR_NAME=VALUE** parameter at the moment of running the container.
...
But how can we initialize variables at build time, in order to simplify the way we run the container, and at the same time make our build processes more maintainable?  
...
Remember, those environment variables, can be overriden later by passing new values to **docker run**.
...
RUN chown -R www-data:www-data /etc/ssl/certs /etc/ssl/private
RUN a2enmod ssl
...
The easiest way to do it its via the **ssl-cert** linux package, that provides the command **make-ssl-cert** to generate fake self-signed SSL certificates. 
...
www-data it's the default user responsible of running any command at run time. 
...
PHP's profiler and tracer tools, where configured at config/php-ini.dev to dump their reports as files.  
...
Like if you where typing **cd** on the console, **WORKDIR** changes the current folder of your image.  
...
<Code language="bash" 
snippet={`docker build 
    -t dockerhub-username/awesome-php-image:1.0-development 
    --target dev-image .`} />
...
snippet={`
REPOSITORY                              TAG                 IMAGE ID            CREATED             SIZE
dockerhub-username/awesome-php-image    1.0-development     14699ea162d0        2 minutes ago       454MB
`} />
...
snippet={`docker run --rm 
    ...
    dockerhub-username/awesome-php-image:1.0-development`} 
/>
...
<Code 
language="bash" 
snippet={`docker push dockerhub-username/awesome-php-image`}
/>
