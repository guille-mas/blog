...
You need **Docker engine** ( version 19.03 or higher ), and **GNU Make**.
...
Docker allows you to pack your applications around a runtime environment of your choice, provision your environment once, and reuse it everywhere. 
Containers are lighter than virtual machines and share the kernel of their host; Usually run a single process, packed with the minimum required resources.  
Instead of long lasting dedicated servers, containerized infrastructure should be ephemeral and disposable. 

##### Images

A Docker image it's the blueprint from where containers are created. An image it's a stopped container. 
