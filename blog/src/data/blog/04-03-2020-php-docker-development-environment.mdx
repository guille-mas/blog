...
At Imaginary startup, a small group of engineers work together on a tight loop, delivering daily changes to their SaaS source code.  
Today, a new service for their customers, will be deployed to production.  
This service requires the implementation of new endpoints on their API, which are expected to process a 10% of their total requests, consuming several more resources than other enpoints.
The team decides to split the production infrastructure cluster in charge of handling their API.
...
So the day has come to deliver the service their customers expect, and they have been working out a semi automated process to deploy it.

<br/>

The traffic must split between two clusters of groups of containerized API workers, running behind a load balancer, operating at the 7th OSI layer.
