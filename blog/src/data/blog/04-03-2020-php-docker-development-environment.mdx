...
**COPY** creates a new layer, containing every copied file.  
Because docker build only those layers that changed since the last built, and every subsequent layer. To speed up your builds, order the build process implemented in a Dockerfile, to create those layers that change most often, at the bottom of the Dockerfile. 
Usually, the layer that change more often in the life of a docker image, its the sourcecode of the application. Infrastructure changes are not so common as the software they run.  
Because of this, the layer where your sourcecode its copied to a docker image, it's usually one of the last steps of your image build process.

<br/>

> To speed up your image's build process, copy your sourcecode as late as possible
...
Since you set www-data as the non-root user to run your image, you will have to choose port numbers grether than 1024 to expose in your container.  
...
You can now use this Dockerfile to build your custom image, every time your sourcecode or development infrastructure changes:
...
On a third post, I'll show you how to automate the creation of production ready, auto-scalable, AWS ECS infrastructure. 
And how to deploy your docker production images to the cloud, using Terraform and GNUMake.

<br/>

Keep in mind I will need some extra time to write both guides, because my youngest son as born a few days ago. And I won't work for at least the next month on this website.  
