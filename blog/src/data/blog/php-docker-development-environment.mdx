...
Or because I am curious about the business and organizational side of most projects I get involved. 
After all, human time is more expensive than computing time. And for good reasons.  
So, being empathetic while building software; Being it a great product, or an internal tool. 
Have a great impact in the business outcomes, people can generate with them.

<br />

In this post I use Docker to wrap a PHP project into a full fledged development environment. 
Designed to help you and your team, become more effective on your day to day duties.  
...
With docker you can pack applications with their own environment, provision it once, and deploy it into any machine running a docker engine. 
...
You need a Windows VM to do the same in other operating systems.
...
When a new container is created from a given image tag, or layer ID, the file system inside, is built by summing every file system portion, inside each layer.

##### Inmutable vs mutable images

Usually, managing state on your applications is harder than not relying on stateless constructs. Because state introduces more variables you have to deal with. 
Such is the strength of a functional approach vs object oriented.  
And the same applies when we write our infrastructure as program.

<br/>
In the context of Docker images. Inmutability happens when an image is built with every file needed by their main process, and you don't modify their state once deployed.  
Instead of modifying the image while it's running, once a new version of the code it's available for deployment, a new image is baked, and replace the old one.  
...
Bridge are single host networks, the default option when running Linux docker images, and the network type used on this tutorial. Their Windows counter part is NAT.  
...
2. Starts a new container labeled as **guillecloud**.
...
In this case, both TAGS for **guillermomaschwitz/blog** belong to the same image, because both share the same IMAGE ID.  
**node:13.1.0-alpine** (or f20a6d8b6721) it's the base image of **guillermomaschwitz/blog**.
...
You can remove the base image in two ways:

<Code language="bash" snippet={`docker image rm f20a6d8b6721`} />

Or

<Code language="bash" snippet={`docker image rm node:13.1.0-alpine`} />
...
In this section, I show you the source code of a working docker development environment, with a trivial php codebase.  
...
This file configures the behaviour of what I consider a decent PHP development environment. It's located in the config/ folder.
...
Every project that you pass to other devs, should have a clear and concise README file in the root folder. I won't cover this file's content, but you can read it's source code as an example on how it should look like.  
What you write on this file should be the minimal amount of information, needed by others to adopt the tool without your guidance.  
...
Then you can open the file in specialized viewer like https://github.com/kuun/xdebug-trace-viewer to read the report.  
